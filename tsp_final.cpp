...
void crear_puntos()
{
    ...
    for(int i = 0; i < cant_ciudades; i++){
        float x = rand()%100 + 1;
        float y = rand()%100 + 1;
        cities_global.push_back(Ciudad(x,y));
        cities_global[i].id = i;
    }
}
...
float distancia_euclediana(Ciudad A, Ciudad B)
{
    ...
    void cruzamiento(vector<int> &p1, vector<int> &p2, vector<int> &h1, vector<int> &h2){///seleccion
        h1.clear(); h2.clear();
        ...
        bool vet1[n];
        bool vet2[n];
        memset(vet1, 0, sizeof(vet1)); memset(vet2, 0, sizeof(vet2));
    ...
    Poblacion evolucionar(){
        ...
        order();
        int cant;
        for(cant = 0; cant < eugenesia && cant < (int)cromosomas.size(); cant++){
            vector<int> nRuta = cromosomas[cant];
            pSiguiente.addRoute( nRuta );
        }

        while(eugenesia < (int)cromosomas.size()){
            vector<int> h1,h2;
            double r;
    ...
    void order(){
        ...
        for(int i = 0; i < (int)cromosomas.size(); i++){
            double noaptitud = getNaptitud(i);
    ...
    vector<int> get_indices_citiesprim(){
        return cromosomas[0];
    }
...
vector<int> get_indexes(vector<Ciudad> Ciudads){
    int n = Ciudads.size();
    Poblacion ACTUAL(totalpob, n, Ciudads);
    ACTUAL.order();
    vector<int> rIndexes = ACTUAL.get_indices_citiesprim();
    return rIndexes;
}
...
vector<Ciudad> genetico_viajero(vector<Ciudad> &Ciudads){
    ...
    //cout << "Ruta con más aptitud en Poblacion inicial - distancia:"<< ACTUAL.getNaptitud(0)<< endl;
    cout << "Nodos:" << endl;
    ...
    for(int iter = 0; iter < numgeneraciones; iter++){
        ...
        ACTUAL = siguiente;
        ACTUAL.order();
        if(iter%100 == 0 || iter == numgeneraciones - 1) cout << (iter+1)/100 <<"\t\t"<< ACTUAL.getNaptitud(0) << endl;
    }
    ACTUAL.order();
...
int main(){
    ...
    int cant;
    cout << "Cuantas ciudades colocar?: "; cin >> cant;
    cout << "cuantas generaciones?: "; cin >> numgeneraciones;
    numgeneraciones *= 100;
    ...
    vector<int> res_index = get_indexes(resultado);
    cout << "-------------------------------------------------------------------------------------------------------------" << endl;
    cout << endl << "Ruta con mejor aptitud, las coordenadas son:"<< endl;
    for(int i = 0; i < resultado.size(); i++) {
        cout<< resultado[i].x << " " << resultado[i].y << endl;
    }
    cout << "La mejor ruta encontrada tiene ordenadas asi las ciudades: ";
